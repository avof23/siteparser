...
class Parser:
    ...
    def get_categories(
        self, url: str, id_category: str = None, id_category_l1: str = None
    ):
        ...
        for category in categories:
            try:
                if self.identity_category_l1 in category["class"]:
                    self.parser_result.append(
                        {
                            "name_l1": category.text.strip(),
                            "url": category["href"],
                            "sub_l2": [],
                        }
                    )
                    ...
                    self.parser_result[i]["sub_l2"].append(
                        {"name_l2": category.text.strip(), "url": category["href"]}
                    )
            except KeyError:
                self.parser_result.append(
                    {"name_l1": category.text.strip(), "url": category["href"]}
                )
                i += 1
    ...
    def get_some_products(self, url: str):
        ...
        for product in product_div:
            ...
            if caption.find("p", class_="description"):
                ...
                short_description = "..."
            list_properties = [
                prop.text for prop in caption.find_all("div", re.compile("option"))
            ]
    ...
    def get_products_cat(
        self, id_product: str = None, id_image: str = None, id_properties: str = None
    ):
        ...
        for element in self.parser_result:
            if "sub_l2" in element:
                for sub_l2_element in element["sub_l2"]:
                    sub_l2_element["elements"] = self.get_some_products(
                        sub_l2_element["url"]
                    )
                    time.sleep(180)
            else:
...
site_pars.get_products_cat(
    id_product="product-thumb", id_image="img-responsive", id_properties="caption"
)
